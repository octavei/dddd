...
def get_tick_list():
    ...
    res_keys = ["deploy_number", "deployer", "circulating_supply", "tick_memo", "start_block", "tick", "total_supply"] # "holders", "supply"
    keys_map_rows = [10,         2,           6,              7,     3,            1,       5]
    tick_list = []
    for tick_row in tick_list_rows:
        json_tick = {}
        for kid, key in zip(keys_map_rows, res_keys):
            json_tick[key] = tick_row[kid]
        json_tick["total_blocks"] = tick_row[4] - json_tick["start_block"]  # completed_height-start_height
...
def get_balance_list():
    ...
    balance_list = []
    for row in rows:
        balance_list.append({"user_address":row[0],"tick": tick, "available": row[1], "hold": row[2]})
    if DEBUG_LOG:
...
## 查询用户旗下全体Token余额
# 使用例子：http://test.dota.fyi/v1/get_account_balance?account=5FKmjePh2YT58zzKbuNJEM1vXe7uY3HuCugpNVmt1EWJEv7S
...
def get_account_balance():
    ...
    query = db_interface.drv.query_db(table_name="user_currency_balance",
                                      ...
                                      condition_columns=["user_address"],
                                      condition_values=[account])
    rows = db_interface.drv.fetch_batch_query([query])
    balance = []
    for row in rows:
        balance.append({"user_address":account,"tick": row[0], "available": row[1], "hold": row[2]})
    if DEBUG_LOG:
